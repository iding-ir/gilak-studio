...
export function useResize({
  ...
  const applySizeToDom = useCallback(
    (size: Size) => {
      ...
      if (!el) return
      if (typeof size.w === 'number') el.style.width = `${Math.round(size.w)}px`
      if (typeof size.h === 'number') el.style.height = `${Math.round(size.h)}px`
    },
    [targetRef]
  )
  ...
  const handleResizePointerDown = useCallback(
    ...
    [resizable, onResizeStart, onResizeEnd, clamp, scheduleApply, ctx, id, initialSize, targetRef]
  )
